...
- Marks each position to market using validated Polymarket CLOB prices; then writes to `portfolio_snapshots` (upsert on `(agent_id, snapshot_timestamp)`).

**Price sources & fallbacks**
- Binary market: the CLOB YES token midpoint is authoritative; NO price = `1 - YES`.
- Multi-outcome: use the CLOB token matching `position.side` when token metadata is available.
- Gamma/catalog prices remain useful for prompt/display context and disagreement logging, but they are not the accounting authority.
- Fallback if CLOB is missing/invalid: derive implied YES price from prior `position.current_value / shares`; if side = NO, flip with `1 - value_per_share`.
- Final fallback: use the position entry price only if no prior value exists.
- Every market used during snapshot valuation records price provenance in `market_price_snapshots`.
...
- CLOB gaps (missing/null/NaN/out of range): use prior implied price.
- Gamma/CLOB disagreement: accept CLOB, record both values, and log the anomaly.
- Multi-outcome missing CLOB token/side: use prior implied price.
