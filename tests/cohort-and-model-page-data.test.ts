  ...
  it('normalizes payloads and preserves cohort error messages', async () => {
    vi.stubGlobal('fetch', vi.fn()
      .mockResolvedValueOnce(mockResponse(true, {
        cohort: { id: 'c1', cohort_number: 1, started_at: '2026-03-01T00:00:00.000Z', status: 'active', completed_at: null, methodology_version: 'v1', initial_balance: 10000 },
        agents: [{ id: 'a1', model_id: 'gpt-5.1', model_display_name: 'GPT', model_color: '#10B981', cash_balance: 10000, total_invested: 0, status: 'active', total_value: 10000, total_pnl: 0, total_pnl_percent: 0, brier_score: null, position_count: 0, trade_count: 0, num_resolved_bets: 0 }],
        stats: { week_number: 1, total_trades: 3, total_positions_open: 2, markets_with_positions: 2, avg_brier_score: 0.15 },
        equity_curves: { 'gpt-5.1': [{ date: '2026-03-01', value: 10000 }] },
        recent_decisions: [{ id: 'd1', agent_id: 'a1', cohort_id: 'c1', decision_week: 1, decision_timestamp: '2026-03-02T00:00:00.000Z', action: 'HOLD', reasoning: 'Wait', model_display_name: 'GPT', model_color: '#10B981' }]
      }))
    ...
    await expect(fetchCohortDetailData('c1')).resolves.toEqual({
      ...
      data: {
        ...
        agents: [expect.objectContaining({ id: 'a1' })],
        stats: expect.objectContaining({ total_trades: 3 }),
        equityCurves: { 'gpt-5.1': [{ date: '2026-03-01', value: 10000 }] },
...
describe('model detail page data helper', () => {
  it('returns model detail payloads and preserves non-ok null fallback', async () => {
    vi.stubGlobal('fetch', vi.fn()
      .mockResolvedValueOnce(mockResponse(true, {
        model: { id: 'gpt-5.1', display_name: 'GPT-5.2', provider: 'OpenAI', color: '#10B981' },
        num_cohorts: 2,
        total_pnl: 120,
        avg_pnl_percent: 1.2,
