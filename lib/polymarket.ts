  ...
  try {
    ...
    const simplifiedMarkets = markets
      .map(market => {
        // Validate first
        if (!validateMarketData(market)) {
          console.warn(`Skipping invalid closed market ${market.id || 'unknown'}`);
          return null;
        }
        ...
        if (yesToken) {
          ...
          if (!isNaN(parsedPrice)) {
            ...
            console.warn(`Invalid price for closed market ${market.id}, defaulting to 0.5`);
          }
        }
    ...
    // Log if we filtered out any invalid markets
    if (simplifiedMarkets.length < markets.length) {
      console.warn(`Filtered out ${markets.length - simplifiedMarkets.length} invalid closed markets`);
    }

    return simplifiedMarkets;

  } catch (error: any) {
    console.error('Error fetching closed Polymarket markets:', error.message || error);
    throw error;
  }
}
